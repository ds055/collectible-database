...
const genItemModal = (obj, itemType) => {
    ...
    for (const field in obj){
        ...
        if (field !== "id" && field !== "user_id" && field !== "createdAt" && field !== "updatedAt" && field !== "image" && field !== "holographic") {
        ...
        // if the field is holographic or image, grab the appropriate html from the modal.js
            ...
            const imageElHtml = imageEl(itemType);
...
<div class="flex flex-row w-full justify-between">
    <label class="mt-2 ps-2 pt-1 font-bold text-lg" for="${field}">${capitalizeFirstLetter(field)}:</label>
    <input class="input-holder w-52 m-2 p-1.5 rounded-lg" id="${field}" type="text" value="${value}" placeholder="${getPlaceText(type, field)}"> 
</div>
`
} 

const imageEl = (type) => {  return `
<div class="flex flex-row w-full justify-between">
    <label class="ps-2 mt-3 font-bold text-lg" for="${type}-photo-input-el">Upload Image:</label>
    <input class="w-52 m-2 p-1.5 rounded-lg" id="${type}-photo-input-el" type="file" accept="image/png, image/jpeg, image/jpg"> 
</div>
`
} 
